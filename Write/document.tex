...
TODO add explanation of the alpha constant and the usage of dirchilet noise for additional exploration.
...
This reference machine uses a single RTX 2070S, which is saturated to $100\%$ load by the benchmark. Thus all self-play cost of my experiments is stated as estimated self-play time on that machine and bottlenecked by GPU capcacity on it.
...
\begin{table}[!htbp]
  \centering
    ...
    \midrule
    cpuct          & The $C_{puct}$ constant of AlphaZero, see equation \ref{eq:alpha_zero_u}. \\
  ...
  as desirable as wins, a low value as undesirable as losses. In the context of connect4 with a 7x6 board alphaBase thus needs to be divided by at most $7$ to determine the corresponding $\alpha$ value in the context of the original implementation.\\
    \bottomrule
    \end{tabularx}%
  \label{tab:addlabel}%
...
\caption{Results of the runs to determine a good hyperparameter set.}
...
\subsubsection{Results} \label{s:exexp}
